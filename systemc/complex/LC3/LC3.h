...
public:
	tlm_utils::multi_passthrough_initiator_socket<LC3> isocket;
	tlm_utils::multi_passthrough_initiator_socket<LC3> dsocket;
	sc_in<bool> nReset;	//active low
	sc_inout<bool> nIRQ;	//active low	

	LC3(sc_module_name name);
	~LC3();
	void reset();
	void step();
